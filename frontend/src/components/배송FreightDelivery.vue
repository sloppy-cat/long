<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            화물배송 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            화물배송
        </v-card-title >        
        ...
        <v-card-text>
            <Number label="FreightId" v-model="value.freightId" :editMode="editMode" :inputUI="''"/>
            ...
            <Boolean label="IsRecieved" v-model="value.isRecieved" :editMode="editMode" :inputUI="''"/>
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/freightDeliveries'), this.value)
                        } else {
