<template>
    <v-card outlined>
        <v-card-title>
            FreightDelivery # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>

        <v-card-text>
            <div>
                <Number label="FreightId" v-model="item.freightId" :editMode="editMode" @change="change" />
            </div>
            <div>
            ...
            <div>
                <Boolean label="IsRecieved" v-model="item.isRecieved" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/freightDeliveries/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
